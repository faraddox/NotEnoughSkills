...
public abstract class NESGuiScroll
{
...
//    protected final int slotHeight;
    ...
    public NESGuiScroll(NESGui parent, int width, int height, int top, int bottom, int left, int screenWidth, int screenHeight)
    {
        this.parent = parent;
        this.client = parent.mc;
...
//    }
    ...
    public void handleMouseInput(int mouseX, int mouseY) throws IOException
    {
        ...
        if (scroll != 0)
        {
            this.scrollDistance += (float)((-1 * scroll / 120.0F) * 90);
        }
    }

    abstract protected int setSelectedIndex(int mouseListY);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        int scrollBarRight = this.left + this.listWidth - 4;
        ...
        if (Mouse.isButtonDown(0))
        {
            if (this.initialMouseClickY == -1.0F)
            {
                if (isHovering)
                {
                    ...
                    LOG("MouseListY: " + mouseListY);
                    int slotIndex = setSelectedIndex(mouseListY);
        ...
        GL11.glScissor((int)(left      * scaleW), (int)(client.displayHeight - ((bottom - 4) * scaleH)),
                (int)((listWidth - 10) * scaleW), (int)((viewHeight - 8) * scaleH));
        ...
        {
            int slotTop = baseY + getAllHeightForEntry(slotIdx) + this.headerHeight;
            int slotBuffer = this.getEntryHeight(slotIdx) - border;
...
//            tess.draw();
            ...
            client.getTextureManager().bindTexture(parent.elements);
            parent.drawTexturedModalRect(scrollBarLeft, barTop + 4, 237, 17, 6, 2);
            for (int i = 6; i < height - 6; i++)
                parent.drawTexturedModalRect(scrollBarLeft, barTop + i, 237, 19 + i % 2, 6, 1);
            parent.drawTexturedModalRect(scrollBarLeft, barTop + height - 6, 237, 21, 6, 2);
        } else {
            GlStateManager.color(1f, 1f, 1f, 0.3f);
            client.getTextureManager().bindTexture(parent.elements);
            parent.drawGradientRect(scrollBarLeft, top + 8, scrollBarRight, bottom - 8, 0x666666, 0x666666);
        }
    ...
    protected abstract int getAllHeightForEntry(int slotIdx);

    protected abstract int getEntryHeight(int index);
